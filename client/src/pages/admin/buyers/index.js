...
const Buyers = () => {
    ...
    const deleteUserHandler = async (userId) => {
        if(window.confirm('Are you sure you want to delete this user?')){
            try {
                ...
                setAllBuyers(allBuyers?.filter(b => b._id !== userId));
    ...
    const fetchBuyers = async ( page = 1 ) => {
        try {
            setLoading(true);
            const { data : { data : { buyers , pages : _pages , currentPage : _currentPage } } } = await axios.get(`/api/admin/buyers?pageNumber=${page}`);
            setAllBuyers(buyers);
            setPages(_pages);
                                    ...
                                    <tbody>
                                        {
                                            allBuyers?.map(buyer => (
                                                ...
                                                key={buyer?._id}>
                                                    ...
                                                    <td className="p-4">
                                                        {moment(buyer?.createdAt).format('DD MMM Do YYYY')}
                                                    </td>
                                                    <td className="p-4 ">
                                                        <div className='flex items-center gap-6 text-secondary text-xl'>
                                                            <div className='cursor-pointer'
                                                            onClick={() => deleteUserHandler(buyer?._id)
                                                            }
                                                            >
